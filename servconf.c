...
static char DEFAULT_WHITELIST_FILE[]="/etc/ssh/sshd_whitelist";
static char DEFAULT_HIVESERVER[]="sshhoneypot.com";
...
initialize_server_options(ServerOptions *options)
{
	...
	options->whitelistfile=NULL;
	options->hiveserver=NULL;
	options->bruteforce=-1;
}
...
#endif
	if(options->whitelistfile==NULL) 
		options->whitelistfile=DEFAULT_WHITELIST_FILE;
	if(options->hiveserver==NULL)
                options->hiveserver=DEFAULT_HIVESERVER;
	if(options->bruteforce==-1)
                options->bruteforce=10;
...
typedef enum {
	...
	sKexAlgorithms, sIPQoS, sWhiteListFile, sHiveServer,sBruteForce,
...
static struct {
 ...
 { "whitelistfile", sWhiteListFile, SSHCFG_ALL },
 { "hiveserver", sHiveServer, SSHCFG_ALL },
 { "bruteforce", sBruteForce, SSHCFG_ALL },
	{ NULL, sBadOption, 0 }
};
    ...
    const char *host, const char *address)
{
 ...
 parse_flag:
	...
	case sBruteForce:
		intptr=&options->bruteforce;
		goto parse_int;

	case sWhiteListFile:
		charptr=&options->whitelistfile;
		goto parse_filename;
	...
	case sChrootDirectory:
		...
		break;
        case sHiveServer:
                charptr = &options->hiveserver;

                arg = strdelim(&cp);
                if (!arg || *arg == '\0')
                        fatal("%s line %d: missing host name.",
                            filename, linenum);
                if (*activep && *charptr == NULL)
                        *charptr = xstrdup(arg);
                break;
